...
func (blq *BalanceLogQuery) Order(o ...balancelog.OrderOption) *BalanceLogQuery {
	blq.order = append(blq.order, o...)
	return blq
}

// QueryUser chains the current query on the "user" edge.
func (blq *BalanceLogQuery) QueryUser() *UserQuery {
...
func (blq *BalanceLogQuery) Clone() *BalanceLogQuery {
	...
	return &BalanceLogQuery{
		config:     blq.config,
		ctx:        blq.ctx.Clone(),
		order:      append([]balancelog.OrderOption{}, blq.order...),
...
func (blq *BalanceLogQuery) WithUser(opts ...func(*UserQuery)) *BalanceLogQuery {
	query := (&UserClient{config: blq.config}).Query()
...
//		GroupBy(balancelog.FieldAction).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (blq *BalanceLogQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*BalanceLog, init func(*BalanceLog), assign func(*BalanceLog, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*BalanceLog)
	for i := range nodes {
		if nodes[i].user_balance_logs == nil {
			continue
		}
		fk := *nodes[i].user_balance_logs
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_balance_logs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
