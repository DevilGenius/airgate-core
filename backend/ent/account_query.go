...
func (aq *AccountQuery) QueryGroups() *GroupQuery {
...
func (aq *AccountQuery) QueryProxy() *ProxyQuery {
	query := (&ProxyClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) QueryUsageLogs() *UsageLogQuery {
	query := (&UsageLogClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) WithGroups(opts ...func(*GroupQuery)) *AccountQuery {
	query := (&GroupClient{config: aq.config}).Query()
...
func (aq *AccountQuery) WithProxy(opts ...func(*ProxyQuery)) *AccountQuery {
	query := (&ProxyClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) WithUsageLogs(opts ...func(*UsageLogQuery)) *AccountQuery {
	query := (&UsageLogClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withProxy; query != nil {
		if err := aq.loadProxy(ctx, query, nodes, nil,
			func(n *Account, e *Proxy) { n.Edges.Proxy = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withUsageLogs; query != nil {
...
func (aq *AccountQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Account, init func(*Account), assign func(*Account, *Group)) error {
...
func (aq *AccountQuery) loadProxy(ctx context.Context, query *ProxyQuery, nodes []*Account, init func(*Account), assign func(*Account, *Proxy)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Account)
	for i := range nodes {
		if nodes[i].account_proxy == nil {
			continue
		}
		fk := *nodes[i].account_proxy
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_proxy" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AccountQuery) loadUsageLogs(ctx context.Context, query *UsageLogQuery, nodes []*Account, init func(*Account), assign func(*Account, *UsageLog)) error {
	...
	for _, n := range neighbors {
		fk := n.account_usage_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_usage_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
