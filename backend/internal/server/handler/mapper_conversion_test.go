...
func TestDashboardAndUsageMappers(t *testing.T) {
	minutes := 85.0
	stats := toDashboardStatsResp(appdashboard.Stats{TotalAPIKeys: 2, ClosedAccounts: 4, TodayRequests: 7, TodayImageRequests: 3, RPM1M: 3, TPM1M: 30, RPM10M: 2, TPM10M: 20, TPMPerRPMCoefficient1M: 0.1, TPMPerRPMCoefficient10M: 0.2, AvgFirstEventMs: 120, AvgFirstTokenMs: 240, AvgImageDurationMs: 120000, AccountCostPerMinute1M: 15, AccountCostPerMinute10M: 13.2, UsageEstimates: []appdashboard.UsageEstimate{{Plan: "plus", Windows: []appdashboard.UsageEstimateWindow{{Window: "7d", Status: "ready", DailyGrowthPercent: 16, FullCost: 2000, RemainingCost: &minutes, RemainingMinutes: &minutes}}}}})
	...
	if stats.AccountCostPerMinute1M != 15 || stats.AccountCostPerMinute10M != 13.2 || len(stats.UsageEstimates) != 1 || len(stats.UsageEstimates[0].Windows) != 1 || stats.UsageEstimates[0].Windows[0].RemainingMinutes == nil || *stats.UsageEstimates[0].Windows[0].RemainingMinutes != 85 {
		t.Fatalf("仪表盘用量估算响应异常: %+v", stats)
	}
	...
	trend := toDashboardTrendResp(appdashboard.Trend{
		...
		TopAPIKeys:        []appdashboard.APIKeyTrend{{APIKeyID: 8, Name: "prod", Trend: []appdashboard.APIKeyTrendPoint{{Time: "10:00", Tokens: 7, BilledCost: 1.25}}}},
	})
	...
	if trend.TopAPIKeys[0].Trend[0].BilledCost != 1.25 {
		t.Fatalf("Key Top 12 计费金额映射异常: %+v", trend.TopAPIKeys[0].Trend[0])
	}
