...
func (s *Server) registerRoutes() {
	...
	adminGroup.Use(middleware.JWTAuth(s.jwtMgr, s.db), middleware.AdminOnly())
	{
		...
		adminGroup.POST("/accounts/ban", middleware.PublicRateLimit(20, time.Minute), handlers.CredentialImport.BanAccount)
		...
		adminGroup.GET("/monitor/runtime-features", handlers.Monitor.GetMonitorRuntimeFeatures)
		adminGroup.PATCH("/monitor/runtime-features", handlers.Monitor.UpdateMonitorRuntimeFeatures)
	...
	credentialGroup.Use(middleware.PublicRateLimit(20, time.Minute), middleware.CredentialKeyAuth(s.db))
	{
		...
		credentialGroup.POST("/accounts/ban", handlers.CredentialImport.BanAccount)
	}
